    ...
    async()=>{
        ...
        // if (noteObject.tags && noteObject.tags.length > 0) {
          await db.transaction('rw', db.tag, async () => {
            ...
            const updatedTags = await db.tag.bulkGet(noteObject.tags?.map((tag)=>{
              return tag._id 
            })||[])
            newNote = {
              ...newNote,
...
export const fetchSubSectionsPerProjectId = async (projectId: string) => {
